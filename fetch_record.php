...
$db = new Connect_DB();
$db->connect();
$sql = "select `id`,`name`,`class`,`check_type`,`url`,`url_log`,`policy`,`is_evil`,`talk_title`,`cid_parent` from `country` where `class`";
...
if(!empty($stmt))
{
  foreach($stmt as $item)
  {
    try
    {
      ...
      ${$country} = new $country((int)$item['id'],$item['url'],$item['policy'],(bool)$item['is_evil'],$item['cid_parent'],$item['queue']);
      ${$country}->insert();
      unset(${$country});
    }
    catch(Exception $e)
